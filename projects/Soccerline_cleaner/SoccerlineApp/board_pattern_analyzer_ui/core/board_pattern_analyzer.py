...
# Key columns that get hashed (value replaced with hash)
_KEY_COLUMNS = {"PostAuthorKey", "CommenterKey"}

# PII columns that get dropped entirely when anonymization is ON
_PII_COLUMNS = {
    "Author", "AuthorId", "AuthorIp",
    "AuthorNames", "AuthorIds", "AuthorIps",
    "CommentNickname", "CommentNicknameRaw",
    "CommentUserId", "CommentUserIdParsed",
    "CommentIpParsed", "CommentAuthorIp",
    "CommenterNames",
}


def hash_key(key: str, salt: str) -> str:
    """Hash an identifier key with a salt. Returns 'h:' prefixed 10-char hex digest."""
...
def load_or_generate_salt(config_path: Path) -> str:
    """Load salt from config or generate a new one."""
    config = _load_json(config_path)
    salt = config.get("anonymization_salt")
    if salt:
        return salt
    salt = secrets.token_hex(32)
    config["anonymization_salt"] = salt
    _save_json(config_path, config)
    return salt


def regenerate_salt(config_path: Path) -> str:
...
def apply_anonymization(
    ...
    salt: str,
) -> tuple[dict[str, pd.DataFrame | Path], pd.DataFrame]:
    """Hash key columns and drop PII columns from all DataFrames in results.

    Returns (anonymized_results, mapping_df).
    """
    # Collect all unique keys before hashing
    ...
    for value in results.values():
        ...
        for col in _KEY_COLUMNS:
            if col in value.columns:
                all_keys.update(value[col].dropna().unique())

    # Build mapping
    key_to_hash = {k: hash_key(k, salt) for k in all_keys}

    # Build mapping DataFrame with PII info from merged_time_deltas (richest source)
    mapping_rows = []
    merged = results.get("merged_time_deltas")
    if isinstance(merged, pd.DataFrame) and not merged.empty:
        # Collect author info
        for _, row in merged.drop_duplicates(subset=["PostAuthorKey"]).iterrows():
            key = row.get("PostAuthorKey", "")
            if key and key in key_to_hash:
                mapping_rows.append({
                    "OriginalKey": key,
                    "HashedKey": key_to_hash[key],
                    "Author": row.get("Author", ""),
                    "AuthorId": row.get("AuthorId", ""),
                    "AuthorIp": row.get("AuthorIp", ""),
                })
        # Collect commenter info
        for _, row in merged.drop_duplicates(subset=["CommenterKey"]).iterrows():
            key = row.get("CommenterKey", "")
            if key and key in key_to_hash and key not in {r["OriginalKey"] for r in mapping_rows}:
                mapping_rows.append({
                    "OriginalKey": key,
                    "HashedKey": key_to_hash[key],
                    "Author": row.get("CommentNickname", ""),
                    "AuthorId": row.get("CommentUserId", ""),
                    "AuthorIp": row.get("CommentIpParsed", ""),
                })

    mapping_df = pd.DataFrame(
        mapping_rows,
        columns=["OriginalKey", "HashedKey", "Author", "AuthorId", "AuthorIp"],
    )

    # Apply hashing and drop PII
    anonymized: dict[str, pd.DataFrame | Path] = {}
    for name, value in results.items():
        ...
        for col in _KEY_COLUMNS:
            if col in df.columns:
                df[col] = df[col].map(lambda k: key_to_hash.get(k, k))
        for col in _PII_COLUMNS:
            if col in df.columns:
                df = df.drop(columns=[col])
        anonymized[name] = df

    return anonymized, mapping_df


def write_anonymization_map(
    mapping_df: pd.DataFrame,
    output_path: Path,
) -> Path:
    """Write the anonymization mapping CSV next to the output Excel."""
    map_path = output_path.parent / f"{output_path.stem}_anonymization_map.csv"
...
def analyze_board(
    ...
    if anonymize:
        if not salt:
            raise ValueError("Salt is required when anonymize=True")
        log("익명화 모드 적용 중")
        results, mapping_df = apply_anonymization(results, salt)
        map_path = write_anonymization_map(mapping_df, output)
        log(f"익명화 매핑 파일: {map_path}")
        # Add anonymization info to summary
        summary = results["summary"]
        assert isinstance(summary, pd.DataFrame)
        extra_rows = pd.DataFrame(
            [
                ["Anonymization", "ON"],
                ["Anonymization map", str(map_path)],
                [
                    "주의",
                    "_anonymization_map.csv는 식별자 평문을 포함합니다. "
                    "결과 엑셀과 절대 함께 공유하지 마십시오.",
                ],
            ],
            columns=["Item", "Value"],
        )
        results["summary"] = pd.concat([summary, extra_rows], ignore_index=True)
    else:
        log("익명화 모드 OFF — 평문 식별자가 결과에 포함됩니다")
    ...
    setup_korean_font()
    # interaction_summary is already anonymized if anonymize=True
    vis_data = results["interaction_summary"]
    assert isinstance(vis_data, pd.DataFrame)
    plot_heatmap(vis_data, output.parent, top_keywords)
    plot_network(vis_data, output.parent, top_keywords)

    log(f"Done: {output}")
    results["output_path"] = output
    if map_path:
        results["anonymization_map_path"] = map_path
    return results
